...
With `runrust` Rust files and expressions can be executed just like a shell or Python script. Features include:
...
Install or update `runrust` using Cargo:
...
`runrust` can be installed from the [extra repository](https://archlinux.org/packages/extra/x86_64/runrust/):
...
The primary use for `runrust` is for running Rust source files as scripts. For example:
...
`runrust` will look for embedded dependency and manifest information in the script as shown by the below two equivalent `now.rs` variants:
...
- `--wrapper`: Add a wrapper around the executable. Can be used to run debugging with e.g. `runrust --debug --wrapper rust-lldb my-script.rs` or benchmarking with `runrust --wrapper "hyperfine --runs 100" my-script.rs`

## Executable Scripts

On Unix systems, you can use `#!/usr/bin/env runrust` as a shebang line in a Rust script.  This will allow you to execute a script files (which don't need to have the `.rs` file extension) directly.

If you are using Windows, you can associate the `.ers` extension (executable Rust - a renamed `.rs` file) with `runrust`.  This allows you to execute Rust scripts simply by naming them like any other executable or script.

This can be done using the `runrust --install-file-association` command. Uninstall the file association with `runrust --uninstall-file-association`.
...
$ runrust -e '1+2'
3
$ runrust --dep time --expr "time::OffsetDateTime::now_utc().format(time::Format::Rfc3339).to_string()"`
"2020-10-28T11:42:10+00:00"
$ # Use a specific version of the time crate (instead of default latest):
$ runrust --dep time=0.1.38 -e "time::now().rfc822z().to_string()"
"2020-10-28T11:42:10+00:00"
```
...
You can use `runrust` to write a quick filter, by specifying a closure to be called for each line read from stdin, like so:

```sh
$ cat now.ers | runrust --loop \
...
```sh
$ cat now.ers | runrust --count --loop \
...
The following environment variables are provided to scripts by `runrust`:

- `RUST_SCRIPT_BASE_PATH`: the base path used by `runrust` to resolve relative dependency paths.  Note that this is *not* necessarily the same as either the working directory, or the directory in which the script is being compiled.
